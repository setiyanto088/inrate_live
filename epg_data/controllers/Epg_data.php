...
class Epg_data extends JA_Controller {
  public function __construct()
	...
	function audiencebar_by_channel(){
		...
		$param['get_chnl'] = $this->Anti_si($this->input->post('get_chnl',true));
		...
		$where = ' WHERE (START_TIME BETWEEN "'.$s_date_f.' 00:00:00" AND "'.$e_date_f.' 23:59:59"
		OR END_TIME BETWEEN "'.$s_date_f.' 00:00:00" AND "'.$e_date_f.' 23:59:59"
		)  ';
		
		 if( !empty($param['get_chnl']) ) {
			  $searchtxt = $param['get_chnl'];
			  
			  if($param['get_chnl'] == 'All'){
				$where .= '';
			  }else{ 
				$cjnn = str_replace("_","+",$param['get_chnl']);
				$where .= " AND A.CHANNEL IN ('".str_replace(",","','",$cjnn)."')";
			  }
	...
	function audiencebar_by_channel_file(){
		...
		$where = ' AND A.UPLOAD_DATE BETWEEN "'.$s_date_f.' 00:00:00" AND "'.$e_date_f.' 23:59:59"  ';
	...
	public function index(){
	 ...
	 $test_prod = $this->audience_model->test_new();
      ...
      $this->template->load('maintemplate', 'epg_data/views/audience_view', $data);
	}
	...
	function audience_export(){
		...
		$data_array = json_decode($ndata,true);
