          ...
          <div class="row">
              <div class="col-md-5">
                  <ol class="breadcrumb">
                      ...
                      <li class="breadcrumb-item">Sea Todak Report</li>
                   ...
                   <div class="col-lg-12">	
					<div class="navbar-left" style="padding-left:10px;">
					 ...
					 <div class="navbar-right" style="padding-right:20px;padding-top:10px;">
						<button class="button_white " onclick="compare()" id=''><em class="fa fa-exchange"></em> &nbsp Compare</button>
                          ...
                          <div class="result-table">
                              ...
                              <table aria-describedby="table" id="myTable" class="table table-striped example urate-table">
                                  ...
                                  <tr style="">
                                      ...
                                      <th style="color: #EF0000;" scope="row">Rank</th>
                                      <th style="color: #EF0000;" scope="row">Date</th>
                                      <th style="color: #EF0000;" scope="row">Program</th>
                                      <th style="color: #EF0000;" scope="row">Channel</th>
                                      <th style="color: #EF0000;" scope="row">Genre</th>
                                      <th style="color: #EF0000;" scope="row">Begin Time</th>
                                      <th style="color: #EF0000;" scope="row">End Time</th>
                                      <th style="color: #EF0000;" scope="row">Viewer <img alt="image" src="<?php echo $path; ?>assets/images/icon_arrowdown.png" alt="arrow"></th>
									  <th style="color: #EF0000;" scope="row">Total Views <img alt="image" src="<?php echo $path; ?>assets/images/icon_arrowdown.png" alt="arrow"></th>
									  <th style="color: #EF0000;" scope="row">Duration <img alt="image" src="<?php echo $path; ?>assets/images/icon_arrowdown.png" alt="arrow"></th>
									  <th style="color: #EF0000;" scope="row">Audience <img alt="image" src="<?php echo $path; ?>assets/images/icon_arrowdown.png" alt="arrow"></th>
<th style="color: #EF0000;" scope="row">Reach <img alt="image" src="<?php echo $path; ?>assets/images/icon_arrowdown.png" alt="arrow"></th>
                                  </tr>
                                  </thead>
                      ...
                      <!-- / Tab Table -->
                      <!-- Tab Chart -->
                      <div role="tabpanel" class="tab-pane" id="chart" style="margin-top:60px">  
  ...
  <script language="javascript">
      $(document).ready(function(){           
          ...
          $('#custom_genre').click(function() {   
              ...
              var searchElement = "<div style='padding: 10px; border-left: 1px solid; border-right: 1px solid; background: #fff;' class='search-genre-con'><input type='text' name='search_genre' id='search_genre' class='form-control urate-form-input' value='' onkeyup='search_genre()' paceholder='Search Genre'></div>"; 
          ...
          $('input.toggle-vis').on( 'click', function (e) {
              ...
              $.ajax({
            		url : "<?php echo base_url().'tvpc3dtvsea/listchart_tvpc/'?>" + "?sess_user_id=" + user_id + "&sess_token=" + token  + "&start_date=" + start_date + "&end_date=" + end_date + "&profile=" + profile + "&channel=" + ch + "&stime=" + start_time + "&etime=" + end_time + "&cgroup=" + colgroup,
          ...
          $("#start_date").on("change",function(){
              ...
              $.ajax({
                  type	: "POST",
                  url		: "<?php echo base_url().'tvpc3dtvsea/setprofile/'; ?>"+"?f="+$('#start_date').val(),
                   dataType: 'json',
                  contentType: 'application/json; charset=utf-8',
                  success	: function(response) {
   ...
   function tab_filter(tabs){
	   ...
	   function excel(group=""){
          ...
          if (channel.length < 1) {
		  	...
		  	var form_data = new FormData();  
									...
									$.ajax({
										url: "<?php echo base_url().'tvpc3dtvsea/tvpc3_export'; ?>", 
										dataType: 'text',   
										cache: false,
										contentType: false,
										processData: false,
      ...
      function search(group=""){
          ...
          var table = $("#myTable").DataTable({
              ...
              "processing": true,
              "destroy": true,
              "ajax": "<?php echo base_url().'tvpc3dtvsea/list_tvpc'?>" + "/?sess_user_id=" + user_id + "&sess_token=" + token + "&start_date=" + start_date + "&end_date=" + end_date + "&profile=" + profile + "&channel=" + ch + "&stime=" + start_time + "&etime=" + end_time + "&genre=" + genre,
              "searchDelay": 700,
              "bFilter" : false,
              "aaSorting": [],
              "bInfo" : false,
          ...
          $.ajax({
              url : "<?php echo base_url().'tvpc3dtvsea/listchart_tvpc/'?>" + "?sess_user_id=" + user_id + "&sess_token=" + token  + "&start_date=" + start_date + "&end_date=" + end_date + "&profile=" + profile + "&channel=" + ch + "&stime=" + start_time + "&etime=" + end_time + "&cgroup=" + colgroup + "&genre=" + genre,
              success: function(response) {
      ...
      function create_chart(data,cgroup){
          ...
          for(var i=0; i < channel_header.length; i++){
            ch.push(channel_header[i].replace('&',' AND '));
              ...
              htht = htht+'<div class="col-md-2 col-xs-2"> <span class="dot" style="height: 10px;width: 10px;background-color: '+color_list+';border-radius: 50%;display: inline-block;"></span> '+channel_header[i]+'</div>';
         ...
         // RESET CHART
          $("#myChart").remove();
          $('.result-chart-graph').append('<canvas id="myChart" height="300px"><canvas>');
	  ...
	  function refreshtablefilter(start_date,end_date,profile,ch,start_time,end_time){
          ...
          var table = $("#myTable").DataTable({
              ...
              "processing": true,
              "destroy": true,
              "ajax": "<?php echo base_url().'tvpc3dtvsea/list_tvpc'?>" + "/?sess_user_id=" + user_id + "&sess_token=" + token + "&start_date=" + start_date + "&end_date=" + end_date + "&profile=" + profile + "&channel=" + ch + "&stime=" + start_time + "&etime=" + end_time,
              "searchDelay": 700,
              "bFilter" : false,
              "bInfo" : false,
              "iDisplayLength": 10,
              ...
              "fnInitComplete":function(){
			   ...
			   "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
				...
				<?php foreach($channel as $channelas) { ?>
						if ( aData[3] == "<?php echo $channelas['channel']; ?>" ){ //aData[4]
    ...
    function search_genre(){
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'tvpc3dtvsea/genresearch/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&q=" + query,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
    ...
    function search_channel(){
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'tvpc3dtvsea/channelsearch/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&q=" + query +"&g=" + genre,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
    ...
    function create_daypart(){                  
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'tvpc3dtvsea/setdaypart/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&f=" + from +"&t=" + to,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
                 for(i=0; i < response.length; i++){
    ...
    function search_profile(){
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'tvpc3dtvsea/profilesearch/'; ?>"+"?q="+query+"&f="+period,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
                 $("#profile").next().next().next().empty('');
