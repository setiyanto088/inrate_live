      ...
      <div class="row">
          <div class="col-md-5">
              <ol class="breadcrumb">
                  ...
                  <li class="breadcrumb-item active">Viewing Pattern VOD</li>
              </ol>
              <h3 class="page-title-inner"><strong>Viewing Pattern VOD</strong></h3>
          </div>    
      ...
      <div class="panel">
          <div class="panel-body" style="height: 200px;">
              <div class="row">
					...
					<div class="col-lg-12">	
						...
						<div class="col-lg-3" style="z-index: 999;">	
							<label> Category</label>
								<select class="urate-select" name="genre" id="genre" title="Please Choose Category" required >
						...
						<div class="col-lg-3" style="z-index: 999;">	
							<label> Title</label>
							<select class="urate-select grid-menu" name="channel" id="channel" title="Please choose a Title..." required>
					...
					<div class="col-lg-12" style="position: absolute;top: 215px;">	
						<div class="col-lg-3" >
								<label> Days</label>
								<select class="urate-select" name="days" id="days" title="Please Choose Days" required >
						...
						<div class="col-lg-3" >
								<label> Data</label>
								<select class="urate-select" name="data" id="data" title="Please Choose Days" required >
    	...
    	$(function () {                   
          ...
          $('#from, #to').timepicker({
              ...
              ,change: function(time){
                  ...
                  $.ajax({
                      type	: "POST",
                      url		: "<?php echo base_url().'viewpatternvod/checkdaypart/'; ?>"+"?f="+$('#from').val()+"&t="+$('#to').val(),
                       dataType: 'json',
          ...
          $('#custom_profile').click(function() {   
              ...
              $.ajax({
                  type	: "POST",
                  url		: "<?php echo base_url().'viewpatternvod/setprofile/'; ?>"+"?f="+$('#start_date').val(),
                   dataType: 'json',
                  contentType: 'application/json; charset=utf-8',
                  success	: function(response) {
  ...
  <script language="javascript">
	...
	// window event not working here
	   ...
	   function search_chart(group=""){
          ...
          var form_data = {
              ...
              cgroup     : colgroup
			    ...
			    $.ajax({
				  url : "<?php echo base_url().'viewpatternvod/list_charttvcc'?>",
				  method : "POST",
				  data : form_data,
				  success: function(response) {
 			  ...
 			  $.ajax({
				  url : "<?php echo base_url().'viewpatternvod/list_charttvcc2'?>",
				  method : "POST",
				  data : form_data,
				  success: function(response) {
      ...
      function search(group=""){
          ...
          if(channel == "0"){
 		  ...
 		  channel_header = channel_header.replace(",", "|");
          channel_header=channel_header.split(",");
		     ...
		     $.ajax({
              ...
              success: function(response) {
				  ...
				  if(dts.length == 0){
					  ...
					  $('#panel-blank').html('<img alt="image" class="gambar" src="<?php echo $path9;?>images/Frame388.png" style=" margin-left: auto;margin-right: auto;display: block;" id="sss">');
					  ...
					  for(var t=0; t < dts.length; t++){
						   ...
						   create_chart(dts[t],channel,t,gda[t],data_type);
	   ...
	   function create_chart(data,channel,ts,dates,data_type){
		 ...
		 for(var t=0; t < dts.length; t++){
          ...
          $('.result-chart-graph').append('<div class="titmyChart" ><h3 style="text-align:center"></h3><canvas class="myChart" id="myChart'+ts+'"><canvas></div>');
				 ...
				 tooltips: {
                      callbacks: {
                          label: function(t,d) { 
							  ...
							  return ['TIME : '+dts[t.datasetIndex].TIME[t.index],dtp+' : '+dts[t.datasetIndex].VIEWER[t.index]];
                           }
                      }
                  },
    ...
    function search_genre(){
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'viewpatternvod/genresearch/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&q=" + query,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
	...
	function search_program(){
		...
		if(res.length > 1 || res == '0' ){
			...
			$.ajax({ 
				type	: "POST",
				url		: "<?php echo base_url().'viewpatternvod/programsearch/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&q=" + channel + "&d=" + start_date + "&de=" + end_date,
 				dataType: 'json',
				contentType: 'application/json; charset=utf-8',
				success	: function(response) {
 					$("#program").next().next().next().empty('');
    ...
    function search_channel(){
        ...
        var strVar = "";
            ...
            strVar = "<li data-for='channel'><a href='#' data-real='0' data-id='channel'>All Title</a></li>";
         
        
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'viewpatternvod/channelsearch/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&q=" + query +"&g=" + genre+"&start_date=" + start_date+"&end_date=" + end_date,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
    ...
    function search_profile(){
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'viewpatternvod/profilesearch/'; ?>"+"?q="+query+"&f="+period,
             dataType: 'json',
            contentType: 'application/json; charset=utf-8',
            success	: function(response) {
                 $("#profile").next().next().next().empty('');
