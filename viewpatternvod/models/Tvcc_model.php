...
ORDER BY `DATE`,M1
	...
	public function list_chart_vod($params = array()) {	
		$db = $this->clickhouse->db();
		if($params['days'] == 'all' ){
		$ad = "";
		}elseif($params['days'] == 'weekend' ){
			$ad = " AND DATE_FORMAT(`SPLIT_MINUTES`,'%w') IN (0,6)";
		}else{
			$ad = " AND DATE_FORMAT(`SPLIT_MINUTES`,'%w') IN (1,2,3,4,5) ";
		}
		 ...
		 if($params['genre'] == "0"){
			 if($params['channel'] == "0"){
				 ...
				 $pp = " AND PARAM4 IN (".$params['channel_list'].") ";
			 }
		 }ELSE{
			 if($params['channel'] == "0"){
				 $pp = " AND CATEGORY = '".$params['genre']."' ";
			 }else{
				 $pp = " AND CATEGORY = '".$params['genre']."' AND PARAM4 IN (".$params['channel_list'].") ";
			 }
		 }
	
		IF($params['data'] == 'audience'){

				$sql = "
				SELECT MINUTES,
				count(DISTINCT(CARDNO)) AS `VIEWERS`, 
				0 AS ID_PROFILE FROM ( 
				SELECT CARDNO,formatDateTime(SPLIT_MINUTES,'%T') AS MINUTES FROM INRATE_CDR_VOD_SPLIT A WHERE
				`SPLIT_MINUTES` BETWEEN '".$params['start_date']." 00:00:00' AND '".$params['end_date']." 23:59:59' 
				".$ad."
				".$pp."
				GROUP BY `CARDNO`,`MINUTES`
				) A 
				GROUP BY MINUTES
				ORDER BY MINUTES
				";

    	}else if($params['data'] == 'total_views'){

				$sql = "
				SELECT MINUTES,
				count(DURASI_USEETV_SESS) AS `VIEWERS`, 
				0 AS ID_PROFILE FROM ( 
				SELECT CARDNO,DURASI_USEETV_SESS,formatDateTime(SPLIT_MINUTES,'%T') AS MINUTES FROM INRATE_CDR_VOD_SPLIT A WHERE
				`SPLIT_MINUTES` BETWEEN '".$params['start_date']." 00:00:00' AND '".$params['end_date']." 23:59:59' 
				".$ad."
				".$pp."
				GROUP BY `CARDNO`,DURASI_USEETV_SESS,`MINUTES`
				) A 
				GROUP BY MINUTES
				ORDER BY MINUTES
				";
			...
			$sql = "
				...
				SELECT MINUTES,
					SUM(GAP) AS `VIEWERS`, 
				0 AS ID_PROFILE FROM ( 
				SELECT *,formatDateTime(SPLIT_MINUTES,'%T') AS MINUTES,
				IF(TIMESTAMPDIFF(SECOND,SPLIT_MINUTES,`END_USEETV_SESS`) < 60 ,TIMESTAMPDIFF(SECOND,SPLIT_MINUTES,`END_USEETV_SESS`),60) as GAP
				 FROM INRATE_CDR_VOD_SPLIT A WHERE
				`SPLIT_MINUTES` BETWEEN '".$params['start_date']." 00:00:00' AND '".$params['end_date']." 23:59:59' 
				".$ad."
				".$pp."
				GROUP BY `CARDNO`,`MINUTES`,CHANNEL,CATEGORY
				) A 
				GROUP BY MINUTES
				ORDER BY MINUTES
  ...
  public function channelsearch($strSearch,$strGenre, $start_date , $end_date){ 
  
	 $db = $this->clickhouse->db();
  
        if($strGenre == "0"){
          $strWhere = "AND UPPER(CHANNEL) LIKE '%".strtoupper($strSearch)."%' ";
      }ELSE if($strGenre == ""){
          $strWhere = "AND UPPER(CHANNEL) LIKE '%".strtoupper($strSearch)."%' ";
      }else {
          $strWhere = "AND CATEGORY = '".$strGenre."' AND UPPER(CHANNEL) LIKE '%".strtoupper($strSearch)."%' ";
      }
      
      //     

 $sql = " 
		SELECT DISTINCT B.`PARAM4` AS CHANNEL FROM  `INRATE_CDR_VOD_SPLIT` B  
	  WHERE 1=1 ".$strWhere." 
