...
class Tvprogramun3tvv extends JA_Controller {
  ...
  public function __construct()
	...
	public function filter_days_export(){
		...
		$objWriter->save('/data/opep/www/srcs/html/tmp_doc/filter_days_export.xls');
	   ...
	   $menuL = $this->session->userdata('menuL');
		...
		if(!$this->session->userdata('user_id') || in_array("172",$array_menu) == 0) {
			...
			$this->output->set_content_type('application/json')->set_output(json_encode($result));
			  ...
			  $list = $this->tvprogramun_model->channelsearch($search_t,0,$typerole);
  ...
  public function index()
	{
		...
		$this->template->load('maintemplate', 'tvprogramun3tvv/views/Tvprogramun', $data);
	}	
