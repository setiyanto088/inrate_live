...
class Createuser_model extends CI_Model {
	...
	function listroleall(){
		...
		$query2 = 'SELECT id, role
                      FROM pmt_role WHERE xs1 = 39 ORDER BY xn1';  			
		$sql2	= $this->db->query($query2);
...
public function create_statistic($namaprofile,$data, $people, $user_id) {
	...
	public function edit_pass($params = array()) {
				...
				$sql 	= "UPDATE hrd_profile SET pwd = '".password_hash($params['pwd'], PASSWORD_BCRYPT, $options)."' WHERE id = ".$params['uid'];
	...
	public function save_user($params = array()) {
				...
				$sql 	= "INSERT INTO hrd_profile(id, username, pwd, nama, status_akses, alamat, email, nokontak1, id_role) VALUES(".$id.",'".$params['username']."','".hash("sha512",'unics123')."','".$params['nama']."','1','".$params['alamat']."','".$params['email']."', 0 , ".$params['role'].")";
				if ($sql) {
						...
						if($sc){
							...
							$sql 	= "INSERT INTO t_activation
										(activation_id, user_id, paid_date, doc, create_date, create_by, expired_date)
										VALUES
										(2, ".$id.", NOW(), '".$params['images']."', NOW(), ".$iduser.",  DATE_ADD(NOW(), INTERVAL 14 DAY) )";
							if ($sql) {
	...
	public function list_user_admin($params = array()) {					
		...
		$total_filtered = $this->db->query('SELECT COUNT(id) as total_filtered FROM hrd_profile WHERE status_user  IN (2,3,5)')->row_array();
		$total = $this->db->query('SELECT COUNT(id) as total FROM hrd_profile WHERE status_user  IN (2,3,5)')->row_array();
		$totalall = '';
        if(empty($total)){
            $totalall = 0;
        }else{
	...
	public function save_user_admin($params = array()) {
				...
				$sql 	= "INSERT INTO hrd_profile(id, username, pwd, nama, id_role, status_user, nokontak1, email, status_akses) VALUES(".$id.",'".$params['username']."','".password_hash($params['pwd'], PASSWORD_BCRYPT)."','".$params['nama']."','".$sc."','".$params['role']."','0', '".$params['email']."', 1)";
				if ($sql) {
						return $this->db->query($sql);
