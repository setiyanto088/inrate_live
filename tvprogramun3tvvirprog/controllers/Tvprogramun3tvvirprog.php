...
class Tvprogramun3tvvirprog extends JA_Controller {
  ...
  public function channelsearch(){
	...
	$where = ''; 
		...
		if($type == 'AUDIENCE'){
			...
			$ggg = 'Audience';
					...
					$tbt = 'M_SUM_TV_DASH_PROG_WEEK_PTV_GENRE';
					$tbt_2 = 'M_SUM_TV_DASH_PROG_WEEK_PTV_GENRE';
					$w_week = 1;
					...
					$type_data = 'TOTAL_VIEWS';
					
					$tbt = 'M_SUM_TV_DASH_PROG_WEEK_PTV_GENRE';
					$tbt_2 = 'M_SUM_TV_DASH_PROG_WEEK_PTV_GENRE';
					$w_week = 1;
					$tbttvid = 'M_SUM_TV_DASH_PROG_WEEK_PTV_GENRE';
					$tbttvid_2 = 'M_SUM_TV_DASH_PROG_WEEK_PTV';
					$drag = 'SUM';
		...
		$in_month = '';
 		if($params['week'] == 'All'){
			...
			( 
				SELECT A.PROGRAM, A.GENRE , A.'.$type_data.' AS TOTAL ';
			$week_in2 = "";
			$join_left2 = "";
			$ri2 = 1;
			$th_tb2 = "";
			...
			IF($tipe_filter == 'live'){
				...
				foreach($data['monthdt'] as $wkwkwk){
					...
					$week_in2 = $week_in2.",A".$ri2.".".$type_data." AS M".$ri2." ";
						
					$join_left2 = $join_left2.' LEFT JOIN (
								SELECT * FROM '.$tbt.'
								WHERE `PERIODE` = "'.$wkwkwk['PERIODE_FULL'].'"
								AND SOURCE = "LIVE"
								AND PERIODE_TYPE = "MONTHLY"
								AND ID_PROFILE = 0
								'.$where.'
							) A'.$ri2.' ON A.PROGRAM = A'.$ri2.'.PROGRAM ';
				...
				if($type == 'AUDIENCE'){
							...
							SELECT PROGRAM,GENRE,MAX(AUDIENCE) AUDIENCE, SUM(TOTAL_VIEWS) AS TOTAL_VIEWS, SUM(DURATION) AS DURATION FROM '.$tbt.'
							WHERE `PERIODE` = "'.$tgl.'"
							AND ID_PROFILE = 0
							AND PROGRAM <> ""
							'.$where.'
				...
				foreach($data['monthdt'] as $wkwkwk){
					...
					$join_left2 = $join_left2.' LEFT JOIN (
							...
							) A'.$ri2.' ON A.Rangking = A'.$ri2.'.Rangking ';
		   ...
		   $it1 = 2;
			...
			( SELECT CHN.GENRE,CHN.PROGRAM,CHN.Rangking as rnk, ";
			$week_in = "";
			$in_month_s = "";
			$join_left = "";
			$ri = 1;
			...
			foreach($data['monthdt'] as $wkwk){
				...
				$query_qr = $query_qr." IF(A".$ri.".`VIEWERS` is null,0,A".$ri.".`VIEWERS`) AS WE".$ri.", A".$ri.".`GENRE` AS GENRE".$ri.", A".$ri.".`PROGRAM` AS PROGRAM".$ri.",";
				...
				$join_left = $join_left." 
				LEFT JOIN (	
				SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,PROGRAM DESC) AS Rangking FROM 
												( 
							
							SELECT GENRE,PROGRAM,".$drag."(".$type_data.") AS VIEWERS FROM `".$tbt_2."`
							WHERE `PERIODE_DETAIL` = ".($wkk)."
							AND SUBSTR(PERIODE,1,4) = '".$wkwk['YEAR']."'
							AND ID_PROFILE = 0
							AND PERIODE_TYPE = 'WEEKLY'
							".$where."
							GROUP BY PROGRAM
						) z ORDER BY Rangking
				...
				$th_tbs = $th_tbs."<td>Program</td><td>Genre</td><td>".$ggg."</td>";
				$ri++;
			...
			$query_qr = $query_qr."
				A1.VIEWERS AS TOTAL 
					FROM (
					SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,PROGRAM DESC) AS Rangking FROM (
						SELECT GENRE,PROGRAM, MAX(".$type_data.") AS VIEWERS FROM `".$tbt_2."`
						WHERE `PERIODE_DETAIL` IN (".$week_in.")
						AND SUBSTR(PERIODE,1,4) = '".$data['monthdt'][0]['YEAR']."'
						AND PERIODE_TYPE = 'WEEKLY'
						AND ID_PROFILE = 0
						".$where."
						GROUP BY PROGRAM
		   ...
		   $objPHPExcel->getActiveSheet()->mergeCells('A1:A2');
			...
			foreach($mhtj as $ssss){
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue($array_cell[$gos]."1", "Week ".$gosw." (".$ssss['PER'].")"); 
						...
						->setCellValue($array_cell[$goss].'2', 'Genre')
	...
	public function get_filter_programaud(){
		...
		if($params['week'] == 'All'){
			...
			( 
				SELECT A.PROGRAM PROGRAM, A.GENRE GENRE, A.".$type_data." AS TOTAL ";
			$week_in2 = "";
			$join_left2 = "";
			$ri2 = 1;
			$th_tb2 = "";
		   ...
		   foreach ( $list['data'] as $k => $v ) {
			...
			( SELECT CHN.GENRE GENRE ,CHN.PROGRAM PROGRAM,CHN.Rangking as rnk,  ";
			$week_in = "";
			$in_month_s = "";
			$join_left = "";
			$ri = 1;
			...
			<table id="example3" class="table table-striped example" style="width: 100%">
								<thead style="color:red">
									<tr>
										...
										<th rowspan = "0" >Genre <img class="cArrowDown" src="'.$pathx.'assets/images/icon_arrowdown.png"></th>
										'.$th_tb.'
  ...
  public function index()
	{
		...
		$data['genre'] = $this->tvprogramun_model->list_genre();
		...
		$this->template->load('maintemplate', 'tvprogramun3tvvirprog/views/Tvprogramun', $data);
	}	
							...
							LEFT JOIN (	
								SELECT z.*, rank() over ( ORDER BY VIEWERS DESC,CHANNEL DESC) AS Rangking FROM 
									( 
										...
										WHERE SUBSTR(TANGGAL,1,4) = '".$wkwk['YEAR']."' AND TANGGAL = '".$start_date."-".$bulan[$end_date]."' AND `WEEK` = ".((($wkwk['WEEK']-$w_week)))."
										".$where."
										GROUP BY CHANNEL
									) z ORDER BY Rangking
								) A".$ri." ON CHN.Rangking = A".$ri.".Rangking 
