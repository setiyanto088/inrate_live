...
class Createprofileu_model extends CI_Model {
	...
	public function listprofile($iduser,$idrole) {
		...
		$query = "SELECT a.id,if(flag = 1,concat(name,'*'),name) `name`, grouping,flag, postbuy_status FROM t_profiling_ub_res a where user_id_profil = ".$iduser." order by a.`user_id_profil` ASC, status_job DESC, date_create desc ";
	...
	public function done_p($id){
		...
		$db = $this->clickhouse->db();	
				$sql 	= "SELECT * FROM (SELECT * FROM M_MONTH_PROFILE_RES UNION ALL SELECT * FROM M_MONTH_PROFILE_RES_P22 ) A
							WHERE PROFILE_ID = ".$id."
	...
	public function list_profile($params = array()) {					
		...
		$sql 	= 'SELECT *,a.id as idprofile FROM t_profiling_ub_res a WHERE (status in (1,2,3)) AND user_id_profil IN('.$params['iduser'].',0) '.$filter.'
		...
		free_result($this->db->conn_id);
			...
			$total_filtered = $this->db->query('SELECT COUNT(a.id) as total_filtered FROM t_profiling_ub_res a where (status in (1,2,3)) AND user_id_profil IN('.$params['iduser'].',0)')->row_array();
...
public function create1($data) {
	...
	$final_query_count = 'SELECT COUNT(CARDNO) AS people,SUM(WEIGHT) as populasi, SUM(WEIGHT) as populasi_all FROM ( ' .$query.' ) AF WHERE WEIGHT IS NOT NULL ';
	...
	$sql 	= "INSERT INTO t_profiling_ub_res(`name`,grouping,flag,`status`, people,respondents,respondents_all, user_id,user_id_profil,postbuy_status,mediaplan_status,status_dash_str,status_tvcc_str,status_tvpc_str,cm_status_str,date_create) 
	VALUES('".$data['name']."','".json_encode($data['ress'])."','".$flag."','1','".$people."','".$respondents."','".$respondents_all."', '','".$this->session->userdata('user_id')."','0 %','0 %','0 %','0 %','0 %','0 %','".date("Y-m-d H:i:s")."')";
	...
	if ($sql) {
				...
				$final_query = 'SELECT `CARDNO`, '.$id.' `ID_PROFILE`,`M_TYPE`,`WEIGHT`,`WEIGHT_ALL` FROM ( ' .$query.' ) AF WHERE WEIGHT IS NOT NULL ';
				...
				$this->db2->query($sqlada);
						...
						$script_check ="SELECT TIMESTAMPDIFF(MONTH, '2021-09-01', '".date("Y-m-d")."') as DIFF ";
...
public function create($data,$where,$grouping) {
	...
	$where_s = str_replace('"',"'",$where); 
	$queryid = "SELECT COUNT(RESPID) AS people, SUM(WEIGHT) AS populasi,SUM(WEIGHT) AS populasi_all FROM `URBAN_PROFILE_P22` WHERE tv_genre IS NOT NULL AND WEIGHT <> 0 ".$where_s;
	//echo $queryid;die;
	
	$result = $db->select($queryid);
	$hasilcnt = $result->rows();		
	
	//print_r($hasilcnt);die;
	// $sqliCNT	= $this->db->query($queryid);
	
	// $hasilcnt = $sqliCNT->row(); 
	
	if($hasilcnt[0]['people'] == '' ){
		...
		$respondents = $hasilcnt[0]['populasi'];
		$respondents_all = $hasilcnt[0]['populasi_all'];
		$people = $hasilcnt[0]['people'];
	}
	
	$sql 	= "INSERT INTO t_profiling_ub_res(pid,`name`,grouping,flag,`status`, people,respondents,respondents_all, user_id,user_id_profil,postbuy_status,mediaplan_status,status_dash_str,status_tvcc_str,status_tvpc_str,cm_status_str,date_create) 
	VALUES('1','P22_".$data['name']."','".$grouping."','2','1','".$people."','".$respondents."','".$respondents_all."', '','".$this->session->userdata('user_id')."','0 %','0 %','0 %','0 %','0 %','0 %','".date("Y-m-d H:i:s")."')";
	...
	if ($sql) {
				...
				// $sqlada = 	" INSERT INTO PROFILE_CARDNO_RES 
							 // SELECT RESPID2,".$id." AS ID_PROFILE, 0 AS PO, WEIGHT AS weight, WEIGHT AS weight_all FROM `URBAN_PROFILE_P22` WHERE WEIGHT <> 0 ".$where;
				// $this->db->query($sqlada);
				
				$db->write(" 
					INSERT INTO PROFILE_CARDNO_RES 
					 SELECT RESPID2,".$id." AS ID_PROFILE, 0 AS PO, WEIGHT AS weight, WEIGHT AS weight_all FROM `URBAN_PROFILE_P22` WHERE WEIGHT <> 0 ".$where_s.";
				");

						
						$script_check ="SELECT TIMESTAMPDIFF(MONTH, '2022-12-01', '".date("Y-m-d")."') as DIFF ";
							...
							for($i=0;$i<14;$i++){
								...
								if($i < 3){
									$sql5 = "INSERT INTO M_MONTH_PROFILE_RES_P22 VALUES('".$i0."',NULL,NULL,".$id.",'2') ";
								}else{
									$sql5 = "INSERT INTO M_MONTH_PROFILE_RES_P22 VALUES('".$i0."',NULL,NULL,".$id.",'0') ";
								}
								$this->db->query($sql5);
