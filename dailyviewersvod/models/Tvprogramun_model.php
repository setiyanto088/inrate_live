...
class Tvprogramun_model extends CI_Model {
	...
	public function list_channel() {
		$query = "SELECT DISTINCT B.`CATEGORY` AS channel FROM  `PARAM_VOD` B ORDER BY CATEGORY ";
...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_PTV R WHERE 
	...
	public function list_spot_by_program_all_bar_fix_exp($field,$where,$periode,$pilihaudiencebar,$profile,$check,$first_day,$this_day,$where_k) {
		...
		$query = "SELECT DISTINCT L.`".$field."` as channel, CHANNEL_NAME as CHANNEL,VIEWERS AS Spot FROM P_CHANNEL_ADS_USEETV L  	LEFT JOIN M_SUM_TV_DASH2 R ON L.`CHANNEL_NAME`= R.CHANNEL 
			...
			$query = "SELECT R.CONTENT2 AS channel,AVG(VALUES) AS Spot FROM `SUMMARY_VOD` R
							WHERE TYPE_DATA = 'DAY_CATEGORY_VIEWERS'
							".$where."
							AND R.ID_PROFILE = '".$profile."' ".$wh_chn."
							AND R.CONTENT2 IS NOT NULL 
							GROUP BY R.CONTENT2
							ORDER BY AVG(VALUES) DESC";  	

			}elseif ($pilihaudiencebar=='Duration')	 {

				$query = "SELECT R.CONTENT2 AS channel,AVG(VALUES) AS Spot FROM `SUMMARY_VOD` R
							WHERE TYPE_DATA = 'DAY_CATEGORY_DURATION'
							".$where."
							AND R.ID_PROFILE = '".$profile."' ".$wh_chn."
							AND R.CONTENT2 IS NOT NULL 
							GROUP BY R.CONTENT2
	...
	public function list_spot_by_program_all_bar_fix_t($field,$where,$periode,$pilihaudiencebar,$profile,$check,$first_day,$this_day) {
			...
			if 	($pilihaudiencebar=='GRP')	 {
				$query = "SELECT CHANNEL as channel,grp AS Spot FROM M_SUM_TV_DASH_CHAN_GRP_PTV R WHERE 
				 ...
				 AND R.ID_PROFILE = '".$profile."' ".$wh_chn." 
					...
					$query = "SELECT R.CONTENT3 AS channel2,R.CONTENT2 AS channel,AVG(VALUES) AS Spot FROM `SUMMARY_VOD` R
							WHERE TYPE_DATA = 'DAY_PROGRAM_VIEWERS'
							".$where."
							AND R.ID_PROFILE = '".$profile."' ".$wh_chn."
							AND R.CONTENT2 IS NOT NULL 
							GROUP BY R.CONTENT2,R.CONTENT3
							ORDER BY AVG(VALUES) DESC";  		

			}elseif ($pilihaudiencebar=='Duration')	 {

				$query = "SELECT R.CONTENT3 AS channel2,R.CONTENT2 AS channel,AVG(VALUES) AS Spot FROM `SUMMARY_VOD` R
							WHERE TYPE_DATA = 'DAY_PROGRAM_DURATION'
							".$where."
							AND R.ID_PROFILE = '".$profile."' ".$wh_chn."
							AND R.CONTENT2 IS NOT NULL 
							GROUP BY R.CONTENT2,R.CONTENT3
	...
	public function list_spot_by_program_all_bar_fix($field,$where,$periode,$pilihaudiencebar,$profile,$check,$first_day,$this_day) {
	  ...
	  $sql 	= "DELETE FROM CHANNEL_SAVE_VOD WHERE `CHANNEL_NAME` = '".$params['save_channel_name']."'";
     $this->db2->query($sql);
	 
	  $sql 	= "INSERT INTO CHANNEL_SAVE_VOD(`USERID`,`CHANNEL_NAME`,`CHANNEL_LIST`,MENUS) VALUES('".$params['user_id']."','".$params['save_channel_name']."','".$params['channel']."','0')";
  ...
  public function delete_channels($params){ 
    ...
    public function load_channels($params){ 
          ...
          $query = 'SELECT * FROM CHANNEL_SAVE_VOD WHERE USERID="'.$params['user_id'].'" AND MENUS="0" ORDER BY CHANNEL_NAME ';			
  ...
  public function channel_set($preset,$userid){ 
          ...
          $query = 'SELECT * FROM CHANNEL_SAVE_VOD WHERE USERID="'.$userid.'" AND MENUS="0" AND CHANNEL_NAME = "'.$preset.'" ORDER BY CHANNEL_NAME ';		
...
SELECT CHANNEL ,VIEWERS, DAYS FROM M_SUM_TV_DASH_CHAN_DURATION_DAYS_PTV R WHERE 
	...
	public function list_spot_all_days_t($field,$wheres,$periode,$pilihaudiencebar,$profile,$check,$first_day,$this_day) {
		$db = $this->clickhouse->db();
		if($check == "True"){
				$wh_chn = '';
		}else{
		...
		$query = "SELECT DISTINCT L.`".$field."` as channel, CHANNEL_NAME as CHANNEL,VIEWERS AS Spot FROM P_CHANNEL_ADS_USEETV L  	LEFT JOIN M_SUM_TV_DASH2 R ON L.`CHANNEL_NAME`= R.CHANNEL 
			...
			if 	($pilihaudiencebar=='GRP')	 {
				...
				$query = "SELECT *,VALUES AS VIEWERS,CONTENT2 AS CHANNEL,CONTENT3 AS CHANNEL2,CONTENT1 AS DAYS FROM `SUMMARY_VOD` R
							WHERE TYPE_DATA = 'DAY_PROGRAM_VIEWERS'
							".$where."
							AND R.ID_PROFILE = '".$profile."' ".$wh_chn."
							AND CONTENT2 IS NOT NULL 
							ORDER BY CONTENT1,VALUES DESC";  		
	...
	public function list_spot_all_days($field,$wheres,$periode,$pilihaudiencebar,$profile,$check,$first_day,$this_day) {
		...
		$query = "SELECT DISTINCT L.`".$field."` as channel, CHANNEL_NAME as CHANNEL,VIEWERS AS Spot FROM P_CHANNEL_ADS_USEETV L  	LEFT JOIN M_SUM_TV_DASH2 R ON L.`CHANNEL_NAME`= R.CHANNEL 
			...
			if 	($pilihaudiencebar=='GRP')	 {
				...
				$query = "SELECT *,VALUES AS VIEWERS,CONTENT2 AS CHANNEL,CONTENT1 AS DAYS FROM `SUMMARY_VOD` R
							WHERE TYPE_DATA = 'DAY_CATEGORY_VIEWERS'
							".$where."
							AND R.ID_PROFILE = '".$profile."' ".$wh_chn."
							AND CONTENT2 IS NOT NULL 
	...
	public function get_week_days($where){
		...
		$query = 'SELECT DISTINCT(CONTENT1) AS DAYS FROM `SUMMARY_VOD` R
							WHERE 1=1 
							'.$where.'
							
							AND CONTENT2 IS NOT NULL 
							ORDER BY DAYS,VALUES DESC';   
