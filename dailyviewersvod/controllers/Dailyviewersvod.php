...
class Dailyviewersvod extends JA_Controller {
  ...
  public function __construct()
	...
	public function filter_days_export(){
		...
		$index = 2;
		foreach($data['date'] as $datas){
			
			$objPHPExcel->setActiveSheetIndex(0)
					->setCellValue('A'.$index, $datas['date'])
					->setCellValue('B'.$index, $datas['spot']);
			$index++;
		}
		...
		$objWriter->save('/data/opep/srcs/html/tmp_doc/filter_days_export.xls');
	  ...
	  // $menuL = $this->session->userdata('menuL');
		...
		// }else{
			...
			// $search_t =   str_replace('_','',str_replace('%','',str_replace('\\','',str_replace('"','',str_replace("'","",$this->Anti_si($_GET['q']))))));
			  ...
			  // $genre = str_replace("AND","&",$this->Anti_si($search_t));
			  // $list = $this->tvprogramun_model->channelsearch($search_t,$genre,$typerole);
  ...
  public function index()
	{
		...
		$duration_tvod = $this->tvprogramun_model->get_duration_tvod($periode);
		$duration_all = $this->tvprogramun_model->get_duration($periode);
		$data['duration_ssss'] = ($duration_all[0]['DURATION']/60)+($duration_tvod[0]['DURATION']/60);
		...
		$this->template->load('maintemplate', 'dailyviewersvod/views/Tvprogramun', $data);
	}	
	...
	function audiencebar_by_channel_export(){
		...
		if($preset == "0"){
			...
			$where = "AND CONTENT2 IN (".$str_channel.")"; 
		}
		
		if($week == 'ALL'){
			
			$where = $where."";
			$where = $where." AND R.CONTENT1 BETWEEN '".$first_day."' AND '".$this_day."'";
			
			
		}else{
			
			  $where = $where." AND PERIODE = '".$periode."'";
	...
	function header_change(){
		...
		if($tpe_f == 'TVOD' ){
			...
			$duration_all = $this->tvprogramun_model->get_duration($periode);
			$data['duration'] = ($duration_all[0]['DURATION']/60)+($duration_tvod[0]['DURATION']/60);
	...
	function audiencebar_by_channel_t(){
		...
		if($tipe_filter == 'live'){
			...
			$data['channels'] = $this->tvprogramun_model->list_spot_by_program_all_bar_fix_t("channel_name",$where,$periode,$pilihaudiencebar,"0","True",$first_day,$this_day); 
		...
		for ($i=0;$i<count($dataM);$i++){
			...
			$array_channel[$dataM[$i]['channel2']]['Rangking'] = $i+1;
			$array_channel[$dataM[$i]['channel2']]['Spot'] = $dataM[$i]['Spot'];
			$array_channel[$dataM[$i]['channel2']]['CHANNEL'] = $dataM[$i]['channel'];
			$array_channel[$dataM[$i]['channel2']]['CHANNEL2'] = $dataM[$i]['channel2'];
			
			$scam['Rangking'] = $i+1;
			$scam['Spot'] = $dataM[$i]['Spot'];
			$scam['channel'] = $dataM[$i]['channel'];
			$scam['channel2'] = $dataM[$i]['channel2'];
			$data_cha[] = '"'.$dataM[$i]['channel'].'"';
			$spot_cha[] = $dataM[$i]['Spot'];
			array_push($scama, $scam); 
		}	
		...
		$table_html = '<table id="example42" class="table table-striped table-bordered example" style="width: 100%"><thead><tr><th rowspan = "2" >Rank <img class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th><th rowspan = "2" >Title <img class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th><th rowspan = "2" >Kategory <img class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th><th colspan = "'.count($day_dates).'">Audience </th></tr><tr>';
		foreach($day_dates as $day_datess){
			$table_html = $table_html.'<th >'.$day_datess.'</th>';
		}
		
		if($tipe_filter == 'live'){
			$data['all_data'] = $this->tvprogramun_model->list_spot_all_days_t("channel_name",$where,$periode,$pilihaudiencebar,"0","True",$first_day,$this_day);
		}else{
			$data['all_data'] = $this->tvprogramun_model->list_spot_all_days_tvod("channel_name",$where,$periode,$pilihaudiencebar,"0","True",$first_day,$this_day,$tipe_filter);
		}
		...
		foreach($data['all_data'] as $all_data){
			...
			$array_channel[$all_data['CHANNEL2']][$all_data['DAYS']] = $all_data['VIEWERS'];
		...
		foreach($array_channel as $array_channel_a){
			...
			if(isset($array_channel_a['CHANNEL'])){
				...
				$scam2['Rangking'] = $array_channel_a['Rangking'];
				$scam2['Spot'] = $array_channel_a['Spot'];
				$scam2['channel'] = $array_channel_a['CHANNEL2'];
				$scam2['channel2'] = $array_channel_a['CHANNEL'];
		...
		foreach($scama2 as $adadada){
			...
			$table_html = $table_html.'<tr><td>'.$adadada['Rangking'].'</td><td>'.$adadada['channel'].'</td><td>'.$adadada['channel'].'</td>';
