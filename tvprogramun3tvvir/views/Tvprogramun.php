                ...
                <div class="widget-content">
					...
					<div id="table_program">
						<table aria-describedby="table" id="example3" class="table table-striped table-bordered example" style="width: 100%">
							<thead>
								<tr>
									<th scope="row">Rank <img alt="image" class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th>
									<th scope="row">Program <img alt="image" class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th>
									<th scope="row">Channel <img alt="image" class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th>
									<?php $k = 1; foreach($monthdt as $monthdts){ ?>
									<th scope="row" ><?php echo $monthdts['PERIODE']; ?></th>
									<?php  $k++; } ?>	
									<th rowspan = "0" scope="row" >Total<img alt="image" class="cArrowDown" src="<?php echo $pathx;?>assets/images/icon_arrowdown.png"></th>									
...
<script async >
		...
		function delete_channel(){
			...
			var token = $.cookie(window.cookie_prefix + "token");
			  ...
			  $.ajax({
				  url : "<?php echo base_url().'tvprogramun3tvvir/delete_channels'?>",
				  method : "POST",
				  data : form_data,
		...
		function save_channel_list(){
			...
			var token = $.cookie(window.cookie_prefix + "token");
			  ...
			  $.ajax({
				  url : "<?php echo base_url().'tvprogramun3tvvir/save_channels'?>",
				  method : "POST",
				  data : form_data,
				  success: function(response) {
   ...
   function search_channel(){
        ...
        $.ajax({
            type	: "POST",
            url		: "<?php echo base_url().'tvprogramun3tvvir/channelsearch/';?>"+ "?sess_user_id=" + user_id + "&sess_token=" + token +"&q=" + query +"&g=" + genre,
...
function selectAll(){
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/header_change'; ?>", 
...
function program_change(){
	...
	$('#example3').DataTable({
        ...
        "serverSide": true,
        "destroy": true,
		"ajax": "<?php echo base_url().'tvprogramun3tvvir/get_filter_programaud'?>" + "/?sess_user_id=" + user_id + "&sess_token=" + token + "&periode=<?php echo $tahunselected ?>&pilihprog="+type+ "&tgl2="+tgl+"&week2="+week+"&check="+check+"&tipe_filter_prog="+tipe_filter_prog,
		"searching": true,
		"language": {
            "decimal": ",",
            "thousands": "."
...
function channel_change(){
  ...
  $("#example4_wrapper").append('<div class="datatable-loading" style="position: absolute; background-color: rgb(255, 255, 255); opacity: 0.75; text-align: center; z-index: 10; left: 0px; top: 0px; width: 100%; height: 520px;"><span class="datatable-loading-inner" style="font-weight: bold; position: relative; top: 45%;"><img id="loading" src="<?php echo base_url();?>assets/urate-frontend-master/assets/images/icon_loader.gif"></span></div>');
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/audiencebar_by_channel'; ?>", 
		dataType: 'text',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
...
var search_val = $( "input[aria-controls='example3']" ).val();
  ...
  var user_id = $.cookie(window.cookie_prefix + "user_id");
	...
	var table3 = $('#example3').DataTable({
        ...
        "serverSide": true,
        "destroy": true,
		"ajax": "<?php echo base_url().'tvprogramun3tvvir/get_filter_programaud'?>" + "/?sess_user_id=" + user_id + "&sess_token=" + token + "&periode=<?php echo $tahunselected ?>&pilihprog="+fieldas+ "&tgl2="+tgl2+"&week2="+week2+"&searchtxt="+search_val+"&check=True",
		"searching": true,
		"language": {
            "decimal": ",",
            "thousands": "."
...
function day_view_f(){
		...
		$.ajax({
			url: "<?php echo base_url().'tvprogramun3tvvir/filter_days'; ?>", 
			dataType: 'text',  // what to expect back from the PHP script, if anything
			cache: false,
			contentType: false,
			processData: false,
...
function table1_view(){
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/cost_by_program'; ?>", 
		dataType: 'json',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
...
function audiencebar_view2(){
  ...
  $("#example42_wrapper").append('<div class="datatable-loading" style="position: absolute; background-color: rgb(255, 255, 255); opacity: 0.75; text-align: center; z-index: 10; left: 0px; top: 750px; width: 100%; height: 520px;"><span class="datatable-loading-inner" style="font-weight: bold; position: relative; top: 45%;"><img id="loading2" src="<?php echo base_url();?>assets/urate-frontend-master/assets/images/icon_loader.gif"></span></div>');
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/audiencebar_by_channel42'; ?>", 
		dataType: 'text',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
...
function table2_view(){
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/get_header_tbl'; ?>", 
		dataType: 'json',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
		...
		success: function(data){
			...
			$('#example3').DataTable({
				...
				"serverSide": true,
				"destroy": true,
				"ajax": "<?php echo base_url().'tvprogramun3tvvir/get_filter_programaud'?>" + "/?sess_user_id=" + user_id + "&sess_token=" + token + "&periode=<?php echo $tahunselected ?>&pilihprog="+type+ "&tgl2="+tgl+"&week2="+week+"&check="+check+"&profile="+profile_prog+"&tipe_filter_prog="+tipe_filter_prog+"&preset="+preset3,
				"searching": true,
				"language": {
					"decimal": ",",
					"thousands": "."
...
function view_daypart(){
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/daypart_view'; ?>", 
		dataType: 'text',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
...
function day_view(){
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/day_view'; ?>", 
		dataType: 'text',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
...
function ads_type_view(){
	...
	$.ajax({
		url: "<?php echo base_url().'tvprogramun3tvvir/ads_view'; ?>", 
		dataType: 'text',  // what to expect back from the PHP script, if anything
		cache: false,
		contentType: false,
		processData: false,
