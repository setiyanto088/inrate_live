...
ORDER BY CHANNEL_NAME"; 
	...
	public function list_rank_reg_witel_all_rank($periode){
		...
		$query = "
			SELECT MAX(RNK) AS MAX_RANK FROM (
								SELECT COUNT(*) RNK,AREA FROM `M_SUM_TV_DASH_MINIPACK_AREA_PTV`
								WHERE PERIODE = '".$periode."'
								AND REGIONAL = 'ALL'
								AND BRANCH = 'ALL'
								AND TYPE_PERIODE = 'MONTHLY'
	...
	public function list_rank_prog_reg_all_rank2($periode,$where){
		...
		$query = "
		SELECT MAX(RNK) AS MAX_RANK FROM (
		SELECT COUNT(*) RNK,AREA FROM `M_SUM_TV_DASH_GENRE_PTV`
		WHERE PERIODE = '".$periode."'
		AND REGIONAL = 'ALL'
		AND BRANCH = 'ALL'
		AND TYPE_PERIODE = 'MONTHLY'
	...
	public function list_witel($periode,$regional){
		...
		$query = "
			SELECT REGIONAL,BRANCH, IF(BRANCH = 'ALL',REGIONAL,BRANCH) WITEL FROM `M_SUM_TV_DASH_CHANNEL_AREA_PTV`
									WHERE PERIODE = '".$periode."' 
									AND `AREA` = '".$regional."'
									AND REGIONAL <> ''
									AND REGIONAL <> 'ALL'
									GROUP BY REGIONAL, BRANCH
