...
class dashboardareauseetv extends JA_Controller {
  ...
  public function __construct()
	...
	public function filter_program(){
			...
			foreach($arr_regional as $arr_reg){
				...
				foreach($rank_reg as $rank_regs){
					...
					$array_view[$tr][$arr_reg] = $rank_regs['GENRE'];
					$tr++;
				}
				$ta++;
	...
	public function filter_program_nas(){
			...
			foreach($rank_prog_chart as $rank_prog_chartss){
				...
				$array_view[$tr]['GENRE'] = $rank_prog_chartss['GENRE'];
				$tr++;
	...
	public function filter_channel_reg(){
			...
			foreach($arr_regional_name as $arr_reg){
				...
				//print_r($rank_reg);die;
				$tr = 0;
				for($tt = 0;$tt<10;$tt++){
					
					if (isset($rank_reg[$tt]['CHANNEL'])) {
						$array_view[$tt][$arr_reg['WITEL']] = $rank_reg[$tt]['CHANNEL'];
					}else{
						$array_view[$tt][$arr_reg['WITEL']] = "";
					}
					$tr++;
	...
	public function detail(){
			...
			if($tipe_table == 'channel'){
				...
				foreach($arr_regional as $arr_reg){
					...
					for($rt = 0; $rt < $max_rank; $rt++){
						...
						if (isset($rank_reg[$rt]['GENRE'])) {
							$array_view[$rt][$arr_reg] = $rank_reg[$rt]['GENRE'];
						}else{
							$array_view[$rt][$arr_reg] = "";
						}
		...
		$this->template->load('maintemplate_useetv_area', 'dashboardareauseetv/views/Tvprogramun_detail', $data);
	}
	...
	public function detail_nas(){
		...
		if($tipe_table == 'channel'){
				...
				$rank_reg = $this->tvprogramun_model->list_rank_pack_d($periode,'ALL');
				...
				foreach($rank_reg as $rank_regs){
					...
					$array_view[$tr]['RANK'] = $tr+1; 
					$array_view[$tr]['GENRE'] = $rank_regs['GENRE'];
					$tr++;
				}
		...
		$this->template->load('maintemplate_useetv_area', 'dashboardareauseetv/views/Tvprogramun_detail_nas', $data);
	}
	...
	public function regional(){
		...
		$this->template->load('maintemplate_useetv_area', 'dashboardareauseetv/views/Tvprogramun_reg', $data);
	}		
	  ...
	  public function regional_full()
	{
		...
		if($this->session->userdata('id_unit') == '0'){
			$this->template->load('maintemplate_useetv_area', 'dashboardareauseetv/views/Tvprogramun', $data);
		}else{
			$this->regional();
		}
  ...
  public function index()
	{
		...
		foreach($arr_regional as $arr_reg){
			...
			$rank_reg = $this->tvprogramun_model->list_rank_pack($periode,'ALL');
			$tr = 0;
		...
		if($this->session->userdata('id_unit') == '0'){
			$this->template->load('maintemplate_useetv_area', 'dashboardareauseetv/views/Tvprogramun_nas', $data);
		}else{
			$this->regional();
		}
