...
class Tvprogramun3vod extends JA_Controller {
  ...
  public function channelsearch(){
	...
	$where = ''; 
		...
		if($preset == "0"){
			...
			$where2 = $where2." AND SUBSTRING_INDEX(CONTENT1, '|', 1) IN (".$str_channel.")"; 
		}
		...
		if($type == 'AUDIENCE'){
			...
			$ggg = 'Audience';
					...
					$tbt = 'MONTHLY_TITLE_VIEWERS';
					$tbt2 = 'WEEKLY_TITLE_VIEWERS';
					...
					$tbt = 'MONTHLY_TITLE_DURATION';
					$tbt2 = 'WEEKLY_TITLE_DURATION';
 		...
 		if($params['week'] == 'All'){
			...
			$th_tb2 = "";
				...
				foreach($data['monthdt'] as $wkwkwk){
					...
					$join_left2 = $join_left2." LEFT JOIN (
								SELECT *, CONTENT2 AS PROGRAM, CONTENT1 AS CHANNEL,`VALUES` AS VIEWERS FROM SUMMARY_VOD
								WHERE `PERIODE` = '".$wkwkwk['PERIODE_FULL']."'
								AND TYPE_DATA = '".$tbt."'
								AND ID_PROFILE = 0
								".$where."
					...
					if($type == 'AUDIENCE'){
						...
						$query_qr2 = $query_qr2."".$week_in2." FROM (
							SELECT CONTENT2 AS PROGRAM, CONTENT1 AS CHANNEL,MAX(`VALUES`) AS VIEWERS FROM SUMMARY_VOD
							WHERE `PERIODE` = '".$tgl."'
							AND ID_PROFILE = 0
					...
					$query_qr2 = $query_qr2."".$week_in2." FROM (
							SELECT CONTENT2 AS PROGRAM, CONTENT1 AS CHANNEL,SUM(`VALUES`) AS VIEWERS FROM SUMMARY_VOD
		   ...
		   $it1 = 2;
			...
			foreach($mth as $wkwk){
				...
				$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue($array_cell[$it1].'1', $bulan[$rr].'-'.$params['tgl']);
			$it1++;
			$rr++;
			}
            ...
            header('Content-Disposition: attachment;filename="Audience by Title.xls"');
			...
			$query_qr = "SELECT z.*, rowNumberInAllBlocks()+1 as Rangking FROM 
			( SELECT CHN.CONTENT1,CHN.Rangking as rnk,  ";
			$week_in = "";
			$in_month_s = "";
			$join_left = "";
			$ri = 1;
			...
			foreach($data['monthdt'] as $wkwk){
				...
				$query_qr = $query_qr." IF(A".$ri.".`VIEWERS` is null,0,A".$ri.".`VIEWERS`) AS WE".$ri.", A".$ri.".`CONTENT1` AS CONTENT1".$ri.",";
				...
				SELECT z.*, rowNumberInAllBlocks()+1 as Rangking FROM 
												( 
							SELECT CONTENT1,MAX(`VALUES`) AS VIEWERS FROM `SUMMARY_VOD`
							WHERE `CONTENT3` = '".($wkk)."'
							AND SUBSTR(PERIODE,1,4) = '".$wkwk['YEAR']."'
							AND ID_PROFILE = 0
							AND TYPE_DATA = '".$tbt2."'
							".$where2."
							GROUP BY CONTENT1
				...
				SELECT z.*, rowNumberInAllBlocks()+1 as Rangking FROM 
												( 
							SELECT 
							CONTENT1,MAX(`VALUES`) AS VIEWERS FROM `SUMMARY_VOD`
							WHERE `CONTENT3` = '".($wkk)."'
							AND SUBSTR(PERIODE,1,4) = '".$wkwk['YEAR']."'
							AND ID_PROFILE = 0
							AND TYPE_DATA = '".$tbt2."'
			...
			A1.VIEWERS AS TOTAL 
					...
					SELECT z.*, rowNumberInAllBlocks()+1 as Rangking FROM (
						SELECT CONTENT1
						, MAX(`VALUES`) AS VIEWERS FROM `SUMMARY_VOD`
						WHERE `CONTENT3` IN (".$week_in.")
						AND substring(PERIODE,1,4) = '".$data['monthdt'][0]['YEAR']."'
						AND ID_PROFILE = 0
						AND TYPE_DATA = '".$tbt2."'
						".$where2."
						GROUP BY CONTENT1
		   ...
		   foreach ( $list['data'] as $k => $v ) {
				...
				$ih++;
				for($q=1;$q<=$ri_now;$q++){
					$content = explode("|",$v['CONTENT1'.$q]);
					$scam['program'.$q] =  $content[1];
					$ih++;
					$scam['channel'.$q] =  $content[0];
					$ih++;
					$scam['m'.$q] =  $v['WE'.$q];
					$ih++;
				}
		   ...
		   $objPHPExcel->getActiveSheet()->mergeCells('A1:A2');
			...
			foreach($mhtj as $ssss){
				$objPHPExcel->setActiveSheetIndex(0)->setCellValue($array_cell[$gos]."1", "Week ".$gosw." (".$ssss['PER'].")"); 
						...
						$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue($array_cell[$gos].'2', 'Title')
						->setCellValue($array_cell[$goss].'2', 'Category')
	...
	public function get_filter_programaud(){
		...
		$where2 = ''; 
		 ...
		 if( !empty($this->Anti_si($_GET['periode'])) ) {
		   ...
		   if( !empty($_GET['search']['value']) ) {
			  ...
			  $where =  $where." AND (CONTENT1 LIKE '%".$searchtxt."%' OR CONTENT2 LIKE '%".$searchtxt."%') ";
			  $where2 =  $where2." AND (CONTENT1 LIKE '%".$searchtxt."%' OR CONTENT2 LIKE '%".$searchtxt."%') ";
		  } else {
			  $where =  $where."";
			  $where2 =  $where2."";
		  }
		...
		if($preset == "0"){
			...
			$where = $where." AND CONTENT1 IN (".$str_channel.")"; 
			$where2 = $where2." AND SUBSTRING_INDEX(CONTENT1, '|', 1) IN (".$str_channel.")"; 
		}
		...
		if($params['week'] == 'All'){
		   ...
		   foreach ( $list['data'] as $k => $v ) {
			...
			foreach($data['monthdt'] as $wkwk){
				...
				$th_tb = $th_tb."<th colspan = '3' >Week ".$ri." (".$wkwk['PER'].")</th>";
				$th_tbs = $th_tbs."<td>Title</td><td>Category</td><td>".$ggg."</td>";
				$ri++;
			...
			<table id="example3" class="table table-striped example" style="width: 100%">
								<thead style="color:red">
									<tr>
										...
										<th rowspan = "0" >Title <img class="cArrowDown" src="'.$pathx.'assets/images/icon_arrowdown.png"></th>
										<th rowspan = "0" >Category <img class="cArrowDown" src="'.$pathx.'assets/images/icon_arrowdown.png"></th>
										'.$th_tb.'
  ...
  public function index()
	{
		...
		$this->template->load('maintemplate', 'tvprogramun3vod/views/Tvprogramun', $data);
	}	
	...
	function audiencebar_by_channel42(){
		...
		if($preset == "0"){
			...
			$where = "AND CONTENT1 IN (".$str_channel.")"; 
		}
 		...
 		IF($type == 'AUDIENCE'){
			$tbt2 = 'WEEKLY_CATEGORY';
			$tbt = 'MONTHLY_CATEGORY';
			$tbt2F = 'M_SUM_TV_DASH_CHAN_PTV_WEEK';
			$w_week = 0;
			$tbl_head = 'Audience';
			$drag = 'MAX';
		}ELSEIF($type == 'TOTAL_VIEWS'){
			$tbt2 = 'WEEKLY_CATEGORY_VIEWERS';
			$tbt = 'MONTHLY_CATEGORY_VIEWERS';
			$tbt2F = 'M_SUM_TV_DASH_CHAN_VIEWERS_WEEK_PTV';
			$w_week = 0;
			$tbl_head = 'Total Views';
			$drag = 'SUM';
		}ELSE{
			$tbt2 = 'WEEKLY_CATEGORY_DURATION';
			$tbt = 'MONTHLY_CATEGORY_DURATION';
		...
		if($this_day == "All"){
			...
			if($tipe_filter == 'live'){
				...
				$query_qr2 = "SELECT CHN.CHANNEL as CHANNEL,CHN.VIEWERS AS TOTAL ";
				$week_in2 = "";
				$join_left2 = "";
				$ri2 = 1;
				$th_tb = "";
				...
				foreach($data['monthdt'] as $wkwkwk){
					...
					$join_left2 = $join_left2." LEFT JOIN (
								SELECT *,CONTENT1 AS CHANNEL, `VALUES` AS VIEWERS FROM SUMMARY_VOD
								WHERE `PERIODE` = '".$wkwkwk['PERIODE_FULL']."'
								AND TYPE_DATA = '".$tbt."'
								AND ID_PROFILE = 0
								AND CONTENT1 <> ''
				...
				$in_month = substr($in_month, 0, -1);
				
				IF($type == 'AUDIENCE'){
					if($DATE_NOW == '22' && DATE('Y') == $first_day ){
						$tbt = 'M_SUM_TV_DASH_CHAN_PTV';
						$query_qr2 = $query_qr2."".$week_in2." FROM (
									SELECT *,CONTENT1 AS CHANNEL, `VALUES` AS VIEWERS FROM SUMMARY_VOD
									WHERE `PERIODE` = '".$wkwkwk['PERIODE_FULL']."'
									AND TYPE_DATA = 'MONTHLY_CATEGORY'
									AND ID_PROFILE = 0
									AND CONTENT1 <> ''
									".$where."
						...
						$query_qr2 = $query_qr2."".$week_in2." FROM (
									SELECT *,CONTENT1 AS CHANNEL, `VALUES` AS VIEWERS FROM SUMMARY_VOD
									WHERE `PERIODE` = '".$first_day."'
									AND TYPE_DATA = 'YEARLY_CATEGORY'
									AND ID_PROFILE = 0
									AND CONTENT1 <> ''
									".$where."
								) CHN ".$join_left2." WHERE 1=1  ORDER BY CHN.VIEWERS DESC ";
					...
					$tbt = 'M_SUM_TV_DASH_CHAN_VIEWERS_PTV';
						$query_qr2 = $query_qr2."".$week_in2." FROM (
									SELECT CONTENT1 AS CHANNEL, ".$drag."(`VALUES`) AS VIEWERS FROM SUMMARY_VOD
									WHERE SUBSTR(`PERIODE` ,1,4)= '".$first_day."'
									AND TYPE_DATA = 'MONTHLY_CATEGORY_VIEWERS'
									AND PERIODE IN (".$in_month.")
									AND ID_PROFILE = 0
									".$where."
									GROUP BY CONTENT1
								) CHN ".$join_left2." WHERE 1=1  ORDER BY CHN.VIEWERS DESC ";
					...
					$query_qr2 = $query_qr2."".$week_in2." FROM (
									SELECT CONTENT1 AS CHANNEL, ".$drag."(`VALUES`) AS VIEWERS FROM SUMMARY_VOD
									WHERE SUBSTR(`PERIODE` ,1,4)= '".$first_day."'
									AND TYPE_DATA = 'MONTHLY_CATEGORY_DURATION'
									AND PERIODE IN (".$in_month.")
									AND ID_PROFILE = 0
									AND CONTENT1 <> ''
									".$where."
		...
		// ECHO $query_qr2;DIE;
			...
			foreach($data['monthdt'] as $wkwk){
					...
					IF($ri == 1){
						...
						$join_left = $join_left." 
							LEFT JOIN (	
								SELECT z.*, rowNumberInAllBlocks()+1 as Rangking FROM 
									( 
										SELECT CONTENT1 AS `CHANNEL`,MAX(`VALUES`) AS VIEWERS,PERIODE AS `TANGGAL`,CONTENT3 AS `WEEK`,`ID_PROFILE` FROM SUMMARY_VOD
										WHERE CONTENT2 = '".$wkwk['YEAR']."'  AND CONTENT3 = '".((($wkwk['WEEK']-$w_week)))."'
										AND TYPE_DATA = '".$tbt2."'
										AND CONTENT1 <> ''
										".$where."
										GROUP BY CONTENT1,PERIODE,CONTENT3,ID_PROFILE
										order by VIEWERS desc
									) z ORDER BY Rangking
								) A".$ri." ON CHN.Rangking = A".$ri.".Rangking 
						";
				...
				$th_tbs = $th_tbs."<td>Category</td><td>".$tbl_head."</td>";
				$ri++;
			}
			...
			$ri_last = $ri - 2;
				...
				$query_qr = $query_qr." A1.VIEWERS AS TOTAL
					FROM (
						SELECT z.*, rowNumberInAllBlocks()+1 as Rangking FROM (
						SELECT CONTENT1 AS CHANNEL,MAX(`VALUES`) AS VIEWERS FROM SUMMARY_VOD
						WHERE CONTENT2 = '".$data['monthdt'][0]['YEAR']."' AND CONTENT3 IN (".$week_in.")
						AND CONTENT1 IS NOT NULL
						AND CONTENT1 <> ''
						AND TYPE_DATA = '".$tbt2."'
						".$where."
						GROUP BY CONTENT1
	...
	public function audiencebar_by_channel_export2(){
		...
		if( ! empty($this->input->post('channel',true)) ) {
		   ...
		   $objPHPExcel->setActiveSheetIndex(0)
			...
			$objPHPExcel->getActiveSheet()->mergeCells('A1:A2');
					...
					foreach($data['monthdt'] as $ssss){
						...
						$objPHPExcel->setActiveSheetIndex(0)
						->setCellValue($array_cell[$gos].'2', 'Category')
