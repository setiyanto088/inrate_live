...
class Tvccresp22 extends JA_Controller {
  public function __construct()
	...
	public function index()
	{
		...
		if(!$this->session->userdata('user_id') || in_array("227",$array_menu) == 0) {
          redirect ('/login');
		}
    ...
    $data['genre'] = $this->tvcc_model->list_channel_genre();
		...
		$this->template->load('maintemplate', 'tvccresp22/views/tvcc_view', $data);
	}
  ...
  public function list_tvcc_all(){	     
      ...
      if( !empty($this->Anti_si($_GET['dpart'])) ) {
          $listDaypart = explode(",",$this->Anti_si($_GET['dpart']));
          ...
          if(count($listDaypart) > 1){
              ...
              $arrDaypart = explode("-",$this->Anti_si($_GET['dpart']));
      ...
      if( ! empty($this->Anti_si($_GET['genre'])) ) {
          $genre = str_replace("AND","&",$this->Anti_si($_GET['genre']));
      } else {
          $genre = "0";
      }
