...
class tvprogramun3tvvview extends JA_Controller {
  ...
  public function index()
	{
		...
		if ($data['date'] <> null){
			foreach($data['date'] as $datasss){
				...
				$spot_date_tvod[] = $datasss['spot_tvod'];
				$spot_date_all[] = $datasss['spot_all'];
			}
		}		
		else {
			$data_date[]='';
		...
		$data['json_date'] = $data_date;
		$data['json_spot_date'] = $spot_date;
		$data['json_spot_date_tvod'] = $spot_date_tvod;
		$data['json_spot_date_all'] = $spot_date_all; 
		...
		$this->template->load('maintemplate', 'tvprogramun3tvvview/views/Tvprogramun', $data);
	}	
