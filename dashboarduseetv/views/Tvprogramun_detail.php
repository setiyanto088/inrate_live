    ...
    <div class="content-wrapper">
      ...
      <div class="container-fluid" style="margin-top:50px">
        ...
        <!-- / Dashboard Stats -->
			  ...
			  <div class="col-lg-12" style="" >	
									...
									<div class="col-lg-2" style="text-align:right" >		
										...
										<button id="filter2" type="button" onClick="back('<?php echo $regional_text; ?>')" class="button_black" style="margin-right:5px;"><em class="fa fa-arrow-left" aria-hidden="true"></em> &nbsp Back to Summary</button>
										
										
										
										<?php }else{ ?>
									
										<button id="filter2" type="button" onClick="viewall()" class="button_black" style="margin-right:5px;"><em class="fa fa-arrow-left" aria-hidden="true"></em> &nbsp Back to Summary</button>
		 ...
		 <div class="row">
		    ...
		    <div class="panel urate-panel urate-panel-result" style="border:1px solid #efefef;padding:10px">
              <div class="panel-headings">
                 <div class="pull-left" > <h5 class='urate-panel-title'><strong>Channel Detail</strong></h5>
				 </div>
		...
		<?php }elseif( $tipe_table == 'minipack'){ ?>
		   ...
		   <div class="panel urate-panel urate-panel-result" style="border:1px solid #efefef;padding:10px">
              <div class="panel-headings">
                  <h5 class='urate-panel-title'><strong>Rank Minipack<strong></h5>
              </div>
              <div class="panel-body">
                  <!-- Nav tabs -->
		...
		<?php }else{ ?>
		   ...
		   <div class="panel urate-panel urate-panel-result" style="border:1px solid #efefef;padding:10px">
              <div class="panel-headings">
				  ...
				  <div class="pull-right" ><input type="checkbox" value="fta" id="fta_channel_programcc" checked='checked' onclick="program_change();">Include FTA</label>
				  <button id="filter2xhanne" type="button" onClick="channel_select()" class="button_black" style="margin-left:15px;margin-top:5px;">Select Channel</button>
				  </div>
              ...
              <div class="panel-body">
				...
				<div class="result-table" style="overflow-x:hidden">
								...
								<div id="table_program_pasad">
									<table aria-describedby="mydesc"  id="example3" class="table table-striped" style=" font-size:10px !important;table-layout:fixed;">
										<thead  style="color:red">
		...
		<div class="modal-dialog modal-sm" role="document">
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-titles" id="myModalLabel"><strong>Filter Channel</strong></h4>
				</div>
          ...
          <div id="menusoptmsg">Choose Channel</div>
				...
				<div class="modal-footer">
					...
					<button type="button" class="button_black" id="runjobprocess" onClick="filter_channel_select()"><em class="fa fa-check"></em>&nbsp Process</button>
				</div>
			</div>
		</div>
	</div>
...
$( document ).ready(function() {				
 ...
 $("#filter2_reg").removeClass("menu_link_select");
 $("#filter2_nas").addClass("menu_link");
 $("#filter2_reg").addClass("menu_link");
 
 $("#filter2_reg<?php echo str_replace('0','',$regional_text); ?>").addClass("menu_link_select");
 
  $("#filter_c").val("<?php echo $tipe_table; ?>");
		...
		columns: [
			...
			<?php if($tipe_table == 'program'){ ?>
				...
				<?php }?>
					...
					<?php if($trakos == ""){ ?>
								<?php foreach($list_witel_h as $list_witel_hs){ ?>
									{ data: '<?php echo $list_witel_hs."_PROGRAM" ?>' },	
									{ data: '<?php echo $list_witel_hs."_CHANNEL" ?>' },					
								<?php } ?>
							<?php }else{ ?>
						
								<?php foreach($list_witel_h as $list_witel_hs){ ?>
									{ data: '<?php echo $list_witel_hs."_RANK" ?>' },	
									{ data: '<?php echo $list_witel_hs."_PROGRAM" ?>' },	
									{ data: '<?php echo $list_witel_hs."_CHANNEL" ?>' },					
								<?php } ?>
...
function channel_change_check(){
	

	
}

function filter_period(t){
	
	var url = '<?php echo base_url(); ?>dashboarduseetv/detail';
		var tahun = $('#periode_head').val();
		var filter_c = $('#filter_c').val();
		var bulan = "";
		
		var regional = "<?php echo $regional_text; ?>";

		  
		 $("#laod").append(' <img alt="img" id="loading" src="<?php echo base_url();?>assets/urate-frontend-master/assets/images/icon_loader.gif">');
		  var form = $("<form action='" + url + "' method='post'>" +
			"<input type='hidden' name='tahun' value='" + tahun + "' />" +
			"<input type='hidden' name='filter' value='" + filter_c + "' />" +
...
function program_change(){
	...
	if($('#fta_channel_programcc').is(':checked')){
	 ...
	 $("#table_program_pasad").append(' <img alt="img" id="loading" src="<?php echo base_url();?>assets/urate-frontend-master/assets/images/icon_loader.gif">');
		...
		var tahun = $('#periode_head').val();
			...
			$.ajax({
				url: "<?php echo base_url().'dashboarduseetv/filter_program_detail_a'; ?>", 
				...
				success: function(data){
					...
					$('#table_program_pasad').html(table_html);
	...
	if($('#fta_channel').is(':checked')){
	 ...
	 $("#table_program_channel").append(' <img alt="img" id="loading" src="<?php echo base_url();?>assets/urate-frontend-master/assets/images/icon_loader.gif">');
		...
		var tahun = $('#periode_head').val();
			...
			$.ajax({
				url: "<?php echo base_url().'dashboarduseetv/filter_channel_detail_a'; ?>", 
				dataType: 'json',  // what to expect back from the PHP script, if anything
				cache: false,
				contentType: false,
				processData: false,
