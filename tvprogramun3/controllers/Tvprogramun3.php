  ...
  public function __construct()
	...
	public function audiencebar_by_channel_export_sum(){
		...
		$tipe_filter=$this->Anti_si($this->input->post('tipe_filter_prog',true));
			...
			$period   = $this->getPeriodFromRange($start->format("Y-m-d"),$end->format("Y-m-d"));
			//print_r($period);die;
			$array_period = array();
			...
			foreach ($period as $dt) {
				...
				$totpopulasi = $this->tvprogramun_model->list_populasi2($dt);
				...
				$h12 .= " LEFT JOIN ( SELECT * FROM `M_SUM_TV_DASH_CHAN_PTV` WHERE TANGGAL = '".$dt."' AND ID_PROFILE = '".$profile."' ) B".$iii."
					ON A.CHANNEL = B".$iii.".CHANNEL 
					LEFT JOIN ( SELECT * FROM `M_SUM_TV_DASH_CHAN_EXT_PTV` WHERE TANGGAL = '".$dt."' AND `TPE` = 'TVR' AND ID_PROFILE = '".$profile."' ) C".$iii."
					ON A.CHANNEL = C".$iii.".CHANNEL 
					LEFT JOIN ( SELECT * FROM `M_SUM_TV_DASH_CHAN_EXT_PTV` WHERE TANGGAL = '".$dt."' AND `TPE` = 'TVS' AND ID_PROFILE = '".$profile."' ) D".$iii."
					ON A.CHANNEL = D".$iii.".CHANNEL 
					LEFT JOIN ( SELECT * FROM `M_SUM_TV_DASH_CHAN_VIEWERS_PTV` WHERE TANGGAL = '".$dt."' AND ID_PROFILE = '".$profile."' ) E".$iii."
				...
				$query = " SELECT A.CHANNEL AS CHANNEL, ".$h1." 'aaa' as hoo FROM ( SELECT CHANNEL FROM `M_SUM_TV_DASH_CHAN_PTV` WHERE TANGGAL IN (".substr($wch,0,-1).") AND ID_PROFILE = '".$profile."'
				".$wh_chn."
