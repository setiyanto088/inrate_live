...
class Summaryytd extends JA_Controller {
  ...
  public function index()
	{
		...
		$get_last_day = $this->tvprogramun_model->get_last_day();
		$data['first_day'] = $first_day;
		$data['this_day'] =  $this_day;
		$data['last_day'] = $get_last_day[0]['last_day'];
		$data['start_date'] = $get_last_day[0]['last_day'];
		$data['end_date'] = $get_last_day[0]['last_day'];
		
		$data['spots'] = $this->tvprogramun_model->count_program_all($data);
		$data['jmlchannel'] = $this->tvprogramun_model->count_channel_all($data);
		$data['active_audience'] = $this->tvprogramun_model->get_active_audience_all($data);
		$data['aa'] = $data['active_audience'][0]['VIEWERS'];
		$data['total_views'] = $this->tvprogramun_model->get_total_views_all($data);
		$data['duration'] = $this->tvprogramun_model->get_duration_all($data);
	
		
		
		
		$this->template->load('maintemplate', 'summaryytd/views/Tvprogramun', $data);
	}	
	...
	function audiencebar_by_channel_export(){
		...
		if($week == 'ALL'){
			...
			$where_k = 'AND USER_BEGIN_SESSION BETWEEN "'.$first_day.' 00:00:00" AND "'.$this_day.' 23:59:59"';
			
		}else{
			
			 $where = $where." AND WEEK(DAYS) = ".$week." AND TANGGAL = '".$periode."'";
			 $where_k = 'AND WEEK(USER_BEGIN_SESSION)  = '.$week.' AND DATE_FORMAT(USER_BEGIN_SESSION,"%Y-%M") = "'.$periode.'"';
	...
	function header_change(){
		...
		$params['tpe_f'] =  $this->Anti_si($this->input->post('tpe_f',true));
 		...
 		if($params['tpe_f'] == 'TVOD' ){
			
			$jmlchannel = $this->tvprogramun_model->count_channel_tvod($params);
			$data['jmlchannel'] = number_format($jmlchannel[0]["jmlch"],0,',','.');
			$data['jmlprogram'] = number_format($jmlchannel[0]["jmlpr"],0,',','.');
			$total_views = $this->tvprogramun_model->get_total_views_tvod($params);
			$data['total_views'] = number_format($total_views[0]['TOTAL_VIEWS'],0,',','.');
			$duration = $this->tvprogramun_model->get_duration_tvod($params);
			...
			$active_audience = $this->tvprogramun_model->get_active_audience_tvod($params);
			$data['active_audience'] = number_format($active_audience[0]['VIEWERS'],0,',','.');
		
		}elseif($params['tpe_f'] == 'ALL' ){
		 
			$jmlchannel = $this->tvprogramun_model->count_channel_all($params);
			$data['jmlchannel'] = number_format($jmlchannel[0]["jmlch"],0,',','.');
			$data['jmlprogram'] = number_format($jmlchannel[0]["jmlpr"],0,',','.');
			$total_views = $this->tvprogramun_model->get_total_views_all($params);
			$data['total_views'] = number_format($total_views[0]['TOTAL_VIEWS'],0,',','.');
			$duration = $this->tvprogramun_model->get_duration_all($params);
			$data['duration'] = number_format($duration[0]['DURATION']/60,0,',','.');
			$data['durmin'] = number_format($duration[0]['DURATION']/$total_views[0]['TOTAL_VIEWS'],2,',','.');
			$active_audience = $this->tvprogramun_model->get_active_audience_all($params);
			$data['active_audience'] = number_format($active_audience[0]['VIEWERS'],0,',','.');
		
		}else{
			
			$jmlchannel = $this->tvprogramun_model->count_channel($params);
			$data['jmlchannel'] = number_format($jmlchannel[0]["jmlch"],0,',','.');
			$data['jmlprogram'] = number_format($jmlchannel[0]["jmlpr"],0,',','.');
			$total_views = $this->tvprogramun_model->get_total_views($params);
			$data['total_views'] = number_format($total_views[0]['TOTAL_VIEWS'],0,',','.');
			$duration = $this->tvprogramun_model->get_duration($params);
			$data['duration'] = number_format($duration[0]['DURATION']/60,0,',','.');
			$data['durmin'] = number_format($duration[0]['DURATION']/$total_views[0]['TOTAL_VIEWS'],2,',','.');
			$active_audience = $this->tvprogramun_model->get_active_audience($params);
			$data['active_audience'] = number_format($active_audience[0]['VIEWERS'],0,',','.');
