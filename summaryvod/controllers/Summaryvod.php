...
class Summaryvod extends JA_Controller {
  ...
  public function __construct()
	...
	// public function filter_days_export(){
		...
		// $channelp =  $this->input->post('channelp');
		// $where = '';
		...
		// $objWriter->save('/var/www/html/tmp_doc/filter_days_export.xls');
  ...
  // public function channelsearch(){
		...
		// if(!$this->session->userdata('user_id') || in_array("218",$array_menu) == 0) {
  ...
  public function index()
	{
		...
		$data['total_views_ssss'] = number_format($total_views_all[0]['TOTAL_VIEWS'],0,',','.');
		...
		$data['duration_ssss'] = ($duration_all[0]['DURATION']/60);
		...
		$this->template->load('maintemplate', 'summaryvod/views/Tvprogramun', $data);
	}	
