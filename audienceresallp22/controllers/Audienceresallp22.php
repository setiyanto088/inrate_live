  ...
  public function __construct()
		...
		foreach($array_datas as $array_datasss){
			...
			if($channel == 'ALL'){
				...
				$query_ss = "
					...
					SELECT C.FIELD FIELD,PARENT,B.".$array_datasss['FIELD'][0]." AS ffd,C.LABEL LABEL, A.RESPID RESPID, WEIGHT , WEIGHT_ALL FROM `CDR_EPG_RES_ALL_STEP2_2022` A
					JOIN (SELECT ".$array_datasss['FIELD'][0].",RESPID2 AS RESPID FROM URBAN_PROFILE_P22
					...
					JOIN TREE_PROFILING_RES_P22 B ON A.PARENTID = B.ID) C ON B.".$array_datasss['FIELD'][0]." = C.VALUE 
					WHERE C.FIELD = '".$array_datasss['FIELD'][0]."'
					AND BEGIN_PROGRAM BETWEEN '".$start_date[2]."-".$start_date[1]."-".$start_date[0]." 00:00:00' and '".$end_date[2]."-".$end_date[1]."-".$end_date[0]." 23:59:59'
					".$segment_s."
					GROUP BY RESPID,FIELD,PARENT,ffd,WEIGHT , WEIGHT_ALL,LABEL
					)
					GROUP BY ffd,PARENT,LABEL,`FIELD`
				...
				if($program == 'ALL,ALL'){
					...
					JOIN (SELECT B.LABEL AS PARENT, A.* FROM `TREE_PROFILING_RES_P22` A
					JOIN TREE_PROFILING_RES_P22 B ON A.PARENTID = B.ID) C ON B.".$array_datasss['FIELD'][0]." = C.VALUE 
					JOIN CHANNEL_PARAM_FINAL F ON A.CHANNEL = F.CHANNEL_NAME
					WHERE C.FIELD = '".$array_datasss['FIELD'][0]."'
					AND F.CHANNEL_NAME_PROG = '".str_replace("&amp;","&",$channel)."'
					AND BEGIN_PROGRAM BETWEEN '".$start_date[2]."-".$start_date[1]."-".$start_date[0]." 00:00:00' and '".$end_date[2]."-".$end_date[1]."-".$end_date[0]." 23:59:59'
					".$segment_s."
					GROUP BY RESPID,FIELD,PARENT,ffd,WEIGHT , WEIGHT_ALL,LABEL
					)
			...
			if($program == 'ALL,ALL'){
			 ...
			 $query_ss_all = "
		  	SELECT  COUNT(RESPID) RESP, SUM(WEIGHT) IH, SUM(WEIGHT_ALL) ALLS FROM ( 
				...
				JOIN TREE_PROFILING_RES_P22 B ON A.PARENTID = B.ID) C ON B.AGE_GROUP_RESPID = C.VALUE AND C.FIELD = B.vds
				JOIN CHANNEL_PARAM_FINAL F ON A.CHANNEL = F.CHANNEL_NAME
				WHERE F.CHANNEL_NAME_PROG = '".str_replace("&amp;","&",$channel)."'
				AND BEGIN_PROGRAM BETWEEN '".$start_date[2]."-".$start_date[1]."-".$start_date[0]." 00:00:00' and '".$end_date[2]."-".$end_date[1]."-".$end_date[0]." 23:59:59'
   ...
   public function listsearch(){
	   ...
	   if(!$this->session->userdata('user_id') || in_array("231",$array_menu) == 0) {
        ...
        $list = $this->audience_model->listsearchs($this->Anti_si($search_t), $typerole); 
