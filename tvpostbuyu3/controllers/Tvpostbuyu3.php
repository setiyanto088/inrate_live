...
class Tvpostbuyu3 extends JA_Controller {
  public function __construct()
	...
	public function index()
	{
		...
		$this->template->load('maintemplate', 'tvpostbuyu3/views/Tvpostbuyu', $data);
	}
	...
	function cost_by_program2nn(){
		...
		if($type == "Cost"){			
			$data['programs'] = $this->Tvpostbuyu_model->list_spot_by_program_all2n($field,"COST",$where,$periode);
			
		}elseif($type == "Spot"){
			$data['programs'] = $this->Tvpostbuyu_model->list_spot_by_program_all2n($field,"SPOT",$where,$periode);

		}elseif($type == "GRP"){
			$data['programs'] = $this->Tvpostbuyu_model->list_spot_by_program_all2n($field,"VIEWERS",$where,$periode);
	...
	function cost_by_program2(){
		...
		if($type == "Cost"){			
			$data['programs'] = $this->Tvpostbuyu_model->list_spot_by_program_all($field,"COST",$where,$periode);
		
		}elseif($type == "Spot"){
			$data['programs'] = $this->Tvpostbuyu_model->list_spot_by_program_all($field,"SPOT",$where,$periode);

		}elseif($type == "GRP"){
			$data['programs'] = $this->Tvpostbuyu_model->list_spot_by_program_all($field,"VIEWERS",$where,$periode);
