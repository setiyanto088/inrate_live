...
ORDER BY `DATE`,M1
	...
	$db = $this->clickhouse->db();
			$name_tb = strtoupper(date_format(date_create($arr_dates),"yM")); 
				...
				SELECT 'ALL CHANNEL' AS CHANNELS,'ALL PROGRAM' PROGRAM,SUM(VIEWERS) AS VIEWERS,'ALL PROGRAM'  AS FULL_PROG, SPLIT_MINUTES, formatDateTime(SPLIT_MINUTES,'%R') AS MINUTES,
	...
	$name_tb = strtoupper(date_format(date_create($arr_dates),"yM"));  
		$db = $this->clickhouse->db();
			...
			$sql = "
				SELECT A.*,CONCAT(PROGRAM,'||',formatDateTime(SPLIT_MINUTES,'%Y-%m-%d'),' ',START_TIME) AS FULL_PROG, formatDateTime(SPLIT_MINUTES,'%H:%M') AS MINUTES FROM `RATING_PER_MINUTES_".$name_tb."` A
				JOIN CHANNEL_PARAM_FINAL B ON A.CHANNEL = B.CHANNEL_NAME
				WHERE PROFILE_ID = 0 
				AND formatDateTime(SPLIT_MINUTES,'%Y-%m-%d') = '".$arr_dates."'
 ...
 $sql = "SELECT DISTINCT B.`CHANNEL_NAME_PROG` AS CHANNEL FROM  `CHANNEL_PARAM_FINAL` B  
	  WHERE B.`F2A_STATUS` in (0,-99) ".$strWhere." AND CHANNEL_NAME_PROG IN ('Al Jazeera','Bloomberg','Channel News Asia','CNBC Asia','CNN International','DW TV','Euronews','France 24','SEA Today','TRT World','TVBS News')  ORDER BY CHANNEL_NAME_PROG ";      
