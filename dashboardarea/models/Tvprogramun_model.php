...
//echo $sql;die;
	...
	public function get_week_channel($periode){
		...
		$query = "SELECT DISTINCT `WEEK` as `WEEK` FROM M_SUM_TV_DASH_CHAN_PTV_WEEK WHERE TANGGAL='".$periode."' ORDER BY CAST(`WEEK` AS INT UNSIGNED )";
	...
	public function get_week_program($periode){
		...
		$query = "SELECT DISTINCT `WEEK` as `WEEK` FROM M_SUM_TV_DASH_PROG_WEEK_PTV WHERE TANGGAL='".$periode."' ORDER BY CAST(`WEEK` AS INT UNSIGNED )";
	...
	public function get_last_date(){	
		...
		$sql = "SELECT MAX(DATE) MDATE FROM M_SUM_TV_DASH_CHAN_DAY_AREA";
...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_PTV R WHERE 
	...
	public function list_spot_by_program_all_bar42($field,$where,$periode,$pilihaudiencebar,$profile,$check) {
		$db = $this->clickhouse->db();
	...
	public function list_spot_by_program_all_bar2($field,$where,$periode,$pilihaudiencebar,$profile,$check) {
...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_PTV R WHERE 
	...
	public function list_spot_by_program_all_bar_fix_exp($field,$where,$periode,$pilihaudiencebar,$profile,$check,$first_day,$this_day,$where_k) {
		...
		$query = 'SELECT DISTINCT L.`'.$field.'` as channel, CHANNEL_NAME as CHANNEL,VIEWERS AS Spot FROM P_CHANNEL_ADS_USEETV L  	LEFT JOIN M_SUM_TV_DASH2 R ON L.`CHANNEL_NAME`= R.CHANNEL 
			...
			elseif ($pilihaudiencebar=='avgtotdur')	 {
				...
				$query = 'SELECT R.CHANNEL AS channel,AVG(VIEWERS) AS Spot,VIEWERS_TOTAL FROM `PTV_M_SUM_TV_DASH_CHAN_DAYS` R
							JOIN (
	...
	public function list_spot_by_program_all_bar_fix($query_qr) {
		$db = $this->clickhouse->db();

		$sql	= $db->select($query_qr); 
		return $sql->rows();	   
	}
...
SELECT CHANNEL AS channel,VIEWERS AS Spot FROM M_SUM_TV_DASH_CHAN_DURATION_WEEK_PTV R WHERE 
	...
	public function list_spot_by_program_all2Ps_new($field,$wheres,$params,$pilihprog,$profile,$query_qr2) {
		$db = $this->clickhouse->db();
	...
	public function list_spot_by_program_all2Ps_new_export($field,$wheres,$params,$pilihprog,$profile,$query_qr2) {
			$db = $this->clickhouse->db();
	...
	public function list_spot_by_program_all2Ps_newprog($field,$wheres,$params,$pilihprog,$profile,$query_qr2,$year) {
		$db = $this->clickhouse->db();
		$where = " AND (PROGRAM LIKE '%".$params['searchtxt']."%' OR CHANNEL LIKE '%".$params['searchtxt']."%') ";
	...
	public function list_spot_by_program_all2Ps_newprog_exp($field,$wheres,$params,$pilihprog,$profile,$query_qr2,$year) {
		$db = $this->clickhouse->db();
		$where = " AND (PROGRAM LIKE '%".$params['searchtxt']."%' OR CHANNEL LIKE '%".$params['searchtxt']."%') ";
