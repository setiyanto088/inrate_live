			...
			IF($tipe_filter == 'live'){
				...
				if($type == 'AUDIENCE'){
						...
						$query_qr2 = $query_qr2."".$week_in2." FROM (
							SELECT CHANNEL,PROGRAM,SUM(VIEWERS) AS VIEWERS FROM ".$tbt."  A
							JOIN (
								SELECT `PROGRAM`,CHANNEL FROM M_SUM_TV_DASH_PROG_PTV
				...
				if($type == 'AUDIENCE'){
					...
					$query_qr2 = $query_qr2."".$week_in2." FROM (
						...
						) A ".$join_left2." ) z order by TOTAL DESC";
				}ELSE{
					$query_qr2 = $query_qr2."".$week_in2." FROM (
						SELECT A.CHANNEL,A.PROGRAM, A.VIEWERS + IF(B.VIEWERS IS NULL,0,B.VIEWERS) AS VIEWERS FROM (
									SELECT CHANNEL,PROGRAM,SUM(VIEWERS) AS VIEWERS FROM ".$tbt."
		  ...
		  // }
		   ...
		   if( !empty($_GET['search']['value']) ) {
			  $searchtxt = $_GET['search']['value'];
			   $where =  $where." AND (UPPER(CHANNEL) LIKE '%".strtoupper($searchtxt)."%' OR UPPER(PROGRAM) LIKE '%".strtoupper($searchtxt)."%') ";
		  } else {
			  $where =  $where."";
		  }
...
//$query_qr = $query_qr."A".$ri.".`VIEWERS` AS WE".$ri.",";
			...
			if($tipe_filter == "live"){ 
				...
				$join_left = $join_left." 
				LEFT JOIN (	
							SELECT `CHANNEL`,".$drag."(`VIEWERS`) AS VIEWERS FROM ".$tbl_x." 
		...
		//echo $ri_now;die; 
			...
			if($tipe_filter == "live"){ 
				...
				$query_qr = $query_qr." 0 AS GROWTH,  
						0*100 AS PERSEN
						FROM (
							SELECT DISTINCT(CHANNEL) CHANNEL FROM `".$tbl_x."`
							WHERE SUBSTR(TANGGAL,1,4) = '".$data['weekdt'][0]['YEAR']."' AND `WEEK` IN (".$week_in.")
							AND CHANNEL IS NOT NULL
							 ".$where."
		...
		if($data['area'] ==  'All' && $data['region'] == 'All'){
				...
				foreach($data_array as $data_arrays){
					...
					foreach($data_arrays as $data_arraysr){
						...
						if($region_d[$int_region] !== 'ALL' &&  $region_d[$int_region] !== 'NAME'){
							$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$vtl, ' Region '.$data_arraysr['NAME']);
				...
				foreach($data_array as $data_arrays){
					...
					if($area_d[$int_area] == $data['area']){
						...
						$objPHPExcel->setActiveSheetIndex(0)->setCellValue('A'.$vtl, 'Area '.$data_arrays['NAME']);
						$gos = 0;
						foreach($bulanm as $bulanms){
							$objPHPExcel->setActiveSheetIndex(0)->setCellValue($array_cell[$gos].''.$vtl, $data_arrays['ALL']['ALL'][$bulanms][$data['type']]);
							$gos++;	
